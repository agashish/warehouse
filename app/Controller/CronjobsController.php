...
class CronjobsController extends AppController
{
     ...
     public function beforeFilter()
	{
      ...
      public function getCurrencyrate()
															...
															{	
																...
																if( ( $total + ( $baseRate * $itemPrice ) ) <= 35 )
																{
																	...
																	if( $e == ($orderQuantity-1) )
																	{
																		...
																		$splititem['order_split']		=	"Group B"; //$orderGroup;
																		$splititem['quantity']			=	$inc;
																		$splititem['product_type']		=	"bundle";
																		$splititem['order_id']		=	$result->NumOrderId;
																		$splititem['sku']			=	'S-'.$splitskus[$i];
		 ...
		 * 
		 */ 
		public function mergeSplitOrdersByOrderId_AccordingRules( $rulesOrderId = null )
		{
			...
			if( count( $mergeItems ) > 1 ):
				...
				$e = 0;foreach( $mergeItems as $mergeItemsIndex => $mergeItemsValue ):
				
					if( (($total + $mergeItemsValue->OrderItem->price) * 1.38) <= 35 ):
					
						echo $total  = $mergeItemsValue->OrderItem->price;
						echo "<br>";
						...
						if( $e == (count( $mergeItems ) - 1) ):
							...
							$quantity = 0;
							$total = 0;
							$total  = $mergeItemsValue->OrderItem->price;							
							$quantity = $quantity + $mergeItemsValue->OrderItem->quantity;
