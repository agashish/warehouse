								...
								?> 
									...
									?>
										<tr style="border-bottom: 1px solid #ccc;" >									
											<td>
												<?php 
													if( $item->OrderItem->product_order_id_identify != '' )
													{
														echo $item->OrderItem->product_order_id_identify;
													}
													else
													{
														echo $item->OrderItem->order_id;
													}
												?>
											</td>
											...
											<td>
												<?php
													...
													if( ( $item->OrderItem->product_type == "bundle" && $item->OrderItem->product_sku_identifier == "single" ) )
													{
														if( count($expressOne->Items) == 1 )
														{
															echo ($item->OrderItem->pack_order_quantity * $item->OrderItem->quantity) .'X'. $item->OrderItem->sku;
														}
														else
														{
															echo $item->OrderItem->quantity .'X'. $item->OrderItem->sku;
														}
													}
													else
													{
														if( ( $item->OrderItem->product_type == "single" && $item->OrderItem->product_sku_identifier == "single" ) )
														{
															if( count($expressOne->Items) == 1 )
															{
																echo ($item->OrderItem->quantity) .'X'. $item->OrderItem->sku;
											...
											<td>
												<?php 
													//echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $standardOne->TotalsInfo->Currency), 2, '.' ,''); 													
													if( ( $item->OrderItem->product_type == "bundle" && $item->OrderItem->product_sku_identifier == "single" ) )
													{
														if( count($expressOne->Items) == 1 )
														{
															echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $expressOne->TotalsInfo->Currency), 2, '.' ,''); 													
														}
														else
														{
															echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $expressOne->TotalsInfo->Currency), 2, '.' ,''); 													
													...
													else
													{
														if( ( $item->OrderItem->product_type == "single" && $item->OrderItem->product_sku_identifier == "single" ) )
														{
															if( count($expressOne->Items) == 1 )
															{
																echo number_format($checkproduct->changeCurrency($item->OrderItem->price * $item->OrderItem->quantity, $expressOne->TotalsInfo->Currency), 2, '.' ,''); 													
															}
															else
															{
																echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $expressOne->TotalsInfo->Currency), 2, '.' ,''); 													
											...
											<?php
													...
													else
													{
														if( ( $item->OrderItem->product_type == "single" && $item->OrderItem->product_sku_identifier == "single" ) )
														{
															if( count($expressOne->Items) == 1 )
															{
																echo $item->OrderItem->price * $item->OrderItem->quantity;
															}
															else
															{
																echo $item->OrderItem->price;
											...
											<td>
												<?php 
													if( $item->OrderItem->product_order_id_identify != '' )
													{
														$ord_id = $item->OrderItem->product_order_id_identify;
													}
													else
													{
														$ord_id = $item->OrderItem->order_id;
													} ?>
											...
											<td>
												<?php 
													...
													if( ( $item->OrderItem->product_type == "bundle" && $item->OrderItem->product_sku_identifier == "single" ) )
													{
														if( count($trackedOne->Items) == 1 )
														{
															echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $trackedOne->TotalsInfo->Currency), 2, '.' ,''); 													
														}
														else
														{
															echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $trackedOne->TotalsInfo->Currency), 2, '.' ,''); 													
													...
													else
													{
														if( ( $item->OrderItem->product_type == "single" && $item->OrderItem->product_sku_identifier == "single" ) )
														{
															if( count($trackedOne->Items) == 1 )
															{
																echo number_format($checkproduct->changeCurrency($item->OrderItem->price * $item->OrderItem->quantity, $trackedOne->TotalsInfo->Currency), 2, '.' ,''); 													
															}
															else
															{
																echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $trackedOne->TotalsInfo->Currency), 2, '.' ,''); 													
								...
								?> 
									...
									?>
										<tr style="border-bottom: 1px solid #ccc;" >									
											...
											<td><?php echo $standardOne->ShippingInfo->PostalServiceName; ?></td>								
											<td>
												<?php
													//pack_order_quantity
													//pr($standardOne->Items); exit;
												...
												<?php 
													...
													else
													{
														if( ( $item->OrderItem->product_type == "single" && $item->OrderItem->product_sku_identifier == "single" ) )
														{
															if( count($standardOne->Items) == 1 )
															{
																echo number_format($checkproduct->changeCurrency($item->OrderItem->price * $item->OrderItem->quantity, $standardOne->TotalsInfo->Currency), 2, '.' ,''); 													
															}
															else
															{
																echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $standardOne->TotalsInfo->Currency), 2, '.' ,''); 													
										...
										<tr style="border-bottom: 1px solid #ccc;" >									
											...
											<td>
												<?php
													...
													else if( ( $item->OrderItem->product_type == "bundle" && $item->OrderItem->product_sku_identifier == "multiple" ) )
													{
														if( count($expressOne->Items) == 1 )
														{
															echo $item->OrderItem->sku;
														}
														else
														{
															echo $item->OrderItem->quantity .'X'. $item->OrderItem->sku;
											...
											<td>
												<?php 
													...
													else if( ( $item->OrderItem->product_type == "bundle" && $item->OrderItem->product_sku_identifier == "multiple" ) )
													{
														if( count($expressOne->Items) == 1 )
														{
															echo number_format($checkproduct->changeCurrency((($item->OrderItem->quantity / $item->OrderItem->pack_order_quantity) * $item->OrderItem->price), $expressOne->TotalsInfo->Currency), 2, '.' ,''); 													
														}
														else
														{
															echo number_format($checkproduct->changeCurrency($item->OrderItem->price, $expressOne->TotalsInfo->Currency), 2, '.' ,''); 													
										...
										<tr style="border-bottom: 1px solid #ccc;" >									
											...
											<?php
													...
													else if( ( $item->OrderItem->product_type == "bundle" && $item->OrderItem->product_sku_identifier == "multiple" ) )
													{
														if( count($expressOne->Items) == 1 )
														{
															echo ($item->OrderItem->quantity / $item->OrderItem->pack_order_quantity) * $item->OrderItem->price;
														}
														else
														{
															echo $item->OrderItem->price;
